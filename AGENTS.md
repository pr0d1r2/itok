...
| `zizmor` | the workflow has a security finding: token scope, a persisted credential, an expression pasted into a `run:` block, or an unpinned `uses:` | Fix it. The gate runs `--persona=pedantic`, which is stricter than the minimum on purpose (`V125`), so some findings are smells rather than live bugs -- a smell is still a claim about how the file ages, and fixing it is cheap while the file is in front of you. If the finding is real, understood, and deliberately declined, record it in `.github/zizmor.yml` with the reason AND the condition that would retire it; an ignore with no exit is one nobody revisits. Never reach for `--no-exit-codes` or a broader pattern to make this green. |

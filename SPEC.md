...
V103: **agentic markdown is ONE LINE PER STATEMENT; hard wrap is ⊥, & a line is capped at 1500 chars.** MEASURED: 101 of 101 invariants span >1 line ∴ `grep V47` returns a FRAGMENT & a phrase crossing a wrap returns NOTHING (`the form \`git\`/\`du\`/\`rg\` already use` = 0 hits, split at lines 95/96). grep & every string-anchored edit are LINE-oriented ∴ a wrap defeats both SILENTLY: T85's edit did nothing ∵ `cargo fmt` had rewrapped its anchor, & the suite stayed green. CORRECTNESS, ⊥ a cost trade -- but cost agrees: unwrapping = -706 itok (-2.6%; 1163→439 lines ∴ ~0.97 itok per hard-wrap newline, re-billed EVERY turn, V94) against +38% per one-word diff hunk (265→365 itok) ∵ the line is the DIFF unit & a paragraph re-sends TWICE; break-even ~7 edits/session, & a `/spec` run does 1-3. EXTENDS the table sections' shape to the prose ones, ⊥ invents a rule: 105 of 105 §T/§B rows already obey it. The CAP is the OPPOSITE failure (base64 · minified · a pasted transcript) & sits above the longest legitimate row (B12 = 1124) w/ slack ∴ it forces a REVIEW, ⊥ a shrink (V10's discipline). ⊥ binds code (`rustfmt` max_width 80 owns `.rs`, where a line IS the unit of meaning) | human-facing prose (README wraps on GitHub). This file is hard-wrapped AS WRITTEN ∴ the rule is true only when T91 lands, & rule + checker are ONE commit (V69) -- a guard landing on a file that violates it is RED AT BIRTH.
